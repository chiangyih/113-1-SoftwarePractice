...
while  cap.isOpened(): # 當攝影機有開啟時
    ...
    if results.multi_hand_landmarks: # 如果有偵測到手部
        for i in range(len(results.multi_handedness)): # 逐一取出每一隻手; multi_handedness是手部的資訊, 包含手部的側邊, 可以用來判斷手部是左手還是右手, 以及手部的信心值; 這裡使用len()取得手部的數量
            ...
            hand_landmark_to_list=[] # 建立空串列, 用來存放手部資料格式(轉換後的串列格式)
            ...
            for coordinate_id,joint in enumerate(results.multi_hand_landmarks[i].landmark): # 逐一取出手部的關節座標,results.multi_hand_landmarks是手部的關節座標;coordinate_id是索引值(關節編號),joint是元素值
                ...
                hand_landmark_to_list.append([id,x,y]) # 將手部資料格式轉換為串列格式, 並存入hand_landmark_to_list串列, 這裡使用append()將資料存入串列,id是關節編號,x是x座標,y是y座標
            ...
            # print(finger) #在終端機顯示手指彎曲狀態；finger[]串列中，0表示彎曲，1表示伸直
            
            # ==============================================================================================
            #使用finger[]串列，在終端機顯示哪根手指彎曲，當finger[]=0時，表示彎曲,finger[]=1時，表示伸直，以此類推
            # finger_names = ["Thumb", "Index", "Middle", "Ring", "Pinky"]
            # for idx, bent in enumerate(finger):
            #     if bent == 0:
            #         # print(f"{finger_names[idx]}彎曲") #終端機顯示手指彎曲狀態
            #         cv2.putText(frame_flip, f"{finger_names[idx]} Bending", (10, 30+idx*30), cv2.FONT_HERSHEY_PLAIN, 2, (3,96,240), 2, cv2.LINE_AA) #畫面顯示手指彎曲狀態
            # ==============================================================================================
            
            if finger==[1,1,1,1,1]:
                cv2.putText(frame_flip,"Hi Five",(10,100),cv2.FONT_HERSHEY_SIMPLEX,1,(3,96,240),2,cv2.LINE_AA) #5指張開，畫面顯示Hi Five; 
            if finger==[0,1,1,0,0]:
                cv2.putText(frame_flip,"Hi Peace",(10,100),cv2.FONT_HERSHEY_SIMPLEX,1,(0,250,0),2,cv2.LINE_AA) #食指、中指張開，畫面顯示Hi Peace
            if finger==[0,0,1,1,1]:
                cv2.putText(frame_flip,"It's OK",(10,100),cv2.FONT_HERSHEY_SIMPLEX,1,(200,0,0),2,cv2.LINE_AA) #中指、無名指、小指張開，畫面顯示It's OK
            if finger==[0,0,0,0,0]:
                cv2.putText(frame_flip,"Zero(Fist)",(10,100),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,200),2,cv2.LINE_AA) #5指合攏，畫面顯示Zero(Fist)
            if finger==[0,0,1,0,0]:
                cv2.putText(frame_flip,"XD! It's international Greeting gesture!",(10,100),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,250),2,cv2.LINE_AA)
                #使用opencv繪圖，繪製一個藍色矩形框，範圍為整個手部
                hand_landmark_to_array=np.array(hand_landmark_to_list) # 將手部資料格式轉換為陣列格式, 這裡使用np.array()將資料轉換為陣列
                min_x=int(np.min(hand_landmark_to_array[:,1])) # 取得x座標的最小值, 這裡使用np.min()取得最小值, 並使用int()將座標轉換為整數
                max_x=int(np.max(hand_landmark_to_array[:,1])) # 取得x座標的最大值, 這裡使用np.max()取得最大值, 並使用int()將座標轉換為整數
                min_y=int(np.min(hand_landmark_to_array[:,2])) # 取得y座標的最小值, 這裡使用np.min()取得最小值, 並使用int()將座標轉換為整數
                max_y=int(np.max(hand_landmark_to_array[:,2])) # 取得y座標的最大值, 這裡使用np.max()取得最大值, 並使用int()將座標轉換為整數
                do_mosaic(frame_flip,min_x,min_y,(max_x-min_x),(max_y-min_y)) #呼叫do_mosaic()函數; min_x表示矩形框的左上角x座標, min_y表示矩形框的左上角y座標, (max_x-min_x)表示矩形框的寬度, (max_y-min_y)表示矩形框的高度
